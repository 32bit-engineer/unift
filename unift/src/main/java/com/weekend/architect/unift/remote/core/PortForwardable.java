 ...
 *
 * <p>Implementations (e.g. {@code SshRemoteConnection}) bind a local port on the
 * UniFT server and tunnel all traffic through the underlying SSH transport to the
 * specified remote host/port.  This is used by {@code K8sClientPool} when the
 * Kubernetes API server is not directly reachable from the UniFT host (e.g. when
 * the kubeconfig points to {@code localhost} or a private cluster-internal IP).
 *
 * <pre>
...
public interface PortForwardable {
     ...
     *
     * <p>Passing {@code 0} as the local port lets the OS pick a free port;
     * the actually-bound port is returned.
     *
