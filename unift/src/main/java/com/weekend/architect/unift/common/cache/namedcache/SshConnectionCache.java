 ...
 *
 * <p>Constructed and configured exclusively by {@link RegistryCacheConfig}, which
 * reads the max-size from {@link CacheProperties} ({@code unift.cache.ssh-connection.*}).
 * Injected into {@code SessionRegistry} by Spring using the concrete class as the
 * implicit qualifier — no {@code @Qualifier} annotation needed.
 *
 * <h6>Redis migration</h6>
 * <p>Pass a {@code RedisRegistryCache<String, RemoteConnection>} as the {@code delegate}
 * argument in {@link RegistryCacheConfig#sshConnectionCache()}.  No other changes needed.
 */
