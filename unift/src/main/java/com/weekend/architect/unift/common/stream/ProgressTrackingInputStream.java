...
/**
 * Wraps an {@link InputStream} and fires {@link TransferProgressCallback#onProgress} as bytes are
 * read, reporting cumulative bytes transferred.
 *
 * <p>Used in place of {@link com.jcraft.jsch.SftpProgressMonitor} for downloads to avoid the
 * internal {@code _stat()} call that {@code ChannelSftp.get(path, monitor)} issues when a non-null
 * monitor is provided. That stat call triggers an {@link IndexOutOfBoundsException} in certain
 * mwiede/jsch + OpenSSH server combinations.
 *
