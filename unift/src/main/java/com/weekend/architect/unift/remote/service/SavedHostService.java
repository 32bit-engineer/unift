 ...
 *
 * <p>Supports any {@link com.weekend.architect.unift.remote.enums.ProtocolType} — SSH/SFTP,
 * FTP, S3, Azure Blob, GCS, etc.  All credential fields are encrypted before persistence
 * and decrypted only at connect time — they never appear in any response DTO.
 */
public interface SavedHostService {
     ...
     * @throws com.weekend.architect.unift.remote.exception.CredentialValidationException
     *         if required credentials are missing for the given {@code protocol} / {@code authType}
     */
    SavedHostResponse save(UUID ownerId, SavedHostRequest request);

    /**
     * Returns all saved hosts belonging to the given user, newest first.
    ...
    /**
     * Decrypts the stored credentials on the fly and opens a new remote session
     * for the host's configured protocol.
     * Updates {@code last_used} on success.
     *
     * @return session info (same shape as {@code POST /api/remote/sessions})
     * @throws com.weekend.architect.unift.remote.exception.SavedHostNotFoundException
     *         if not found or not owned by {@code ownerId}
     * @throws com.weekend.architect.unift.remote.exception.ConnectionException
