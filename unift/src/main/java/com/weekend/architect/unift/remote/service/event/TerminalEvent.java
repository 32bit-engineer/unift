 ...
 *
 * <p>Events are partitioned by {@code ownerId} so all events for a given user land on the same
 * partition, preserving chronological ordering for audit/compliance replay.
 *
 * <p>Jackson {@code @JsonTypeInfo} embeds an {@code "eventType"} discriminator field so consumers
 * can deserialize to the correct concrete record without extra configuration.
 */
...
public sealed interface TerminalEvent permits TerminalEvent.SessionOpened, TerminalEvent.SessionClosed {
    ...
    /**
     * Published when a terminal WebSocket session is successfully established and the PTY shell is
     * open.
     *
     ...
     * @param ownerId ID of the authenticated UniFT user
     * @param reason Human-readable close reason (e.g., {@code "client-disconnected"}, {@code
     *     "idle-timeout"})
     * @param timestamp Event creation instant
     */
