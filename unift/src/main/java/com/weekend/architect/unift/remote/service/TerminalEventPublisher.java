 ...
 *
 * <p>Implementations <strong>must never throw</strong> — event publishing failures
 * are logged and swallowed so they never propagate into the WebSocket data path.
 */
public interface TerminalEventPublisher {
    ...
    /**
     * Publishes a {@code terminal.session.closed} event when the terminal closes
     * for any reason.
     *
     * @param session the terminal session that is closing
     * @param reason  human-readable close reason (e.g., {@code "idle-timeout"}, {@code "client-disconnected"})
     */
