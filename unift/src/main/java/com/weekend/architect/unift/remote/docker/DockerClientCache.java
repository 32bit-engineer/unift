...
/**
 * Named cache for live {@link DockerClientEntry} instances — one entry per active SSH session that
 * has requested Docker operations.
 *
 * <p>Constructed exclusively by {@link RegistryCacheConfig}, which reads the max-size from {@link
 * CacheProperties} ({@code unift.cache.docker-client.*}). Injected into {@link DockerClientPool} by
 * concrete-class type matching.
 *
 * <p>{@link DockerClientEntry} holds a live {@code DockerClient} (HTTP connection pool) and an SSH
 * port-forward tunnel — both are stateful, non-serializable resources. This cache is therefore
