...
/**
 * Service interface for Kubernetes cluster management via SSH. All operations execute kubectl CLI
 * commands on the remote host through the session's SSH connection.
 */
public interface K8sService {
    ...
    /**
     * Returns a high-level overview of the cluster: pod/deployment/service/node counts, namespaces,
     * and a sample of recent pods.
     */
    ...
    /**
     * Returns the live YAML for any namespaced resource. {@code managedFields} and {@code status}
     * are stripped for a clean editor view.
     *
    ...
    /**
     * Server-side-applies the given YAML, updating the live resource. Works for any resource type —
     * the kind is determined from the YAML itself.
     *
    ...
    /**
     * Rolls back a Deployment to the specified revision. Pass {@code revision = 0} to roll back to
