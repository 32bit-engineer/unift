 ...
 *       If the value type is publicly visible, use the delegate constructor pattern
 *       (see {@link SshConnectionCache}).  If the value type is package-private, use
 *       the maxSize constructor pattern so the Caffeine backend is built inside the
 *       named-cache class where the type is in scope (see {@link MetricsCache} and
 *       {@link K8sClientCache}).</li>
 ...
 * <p>{@link MetricsCache} wraps the package-private {@code SessionMetrics} type, and
 * {@link K8sClientCache} wraps the package-private {@code K8sClientPool.K8sClientEntry} type.
 * Their constructors therefore accept a {@code long maxSize} (the Caffeine cache is
 * constructed inside each class where the wrapped type is in scope).
 ...
 * {@code K8sClientEntry} holds live socket resources and must remain Caffeine-only
 * regardless (see Redis feasibility table below).
 *
 * <h6>Redis feasibility per cache</h6>
 * <table>
 *   <tr><th>Cache</th><th>Redis?</th><th>Reason</th></tr>
 *   <tr><td>{@link SshConnectionCache}</td><td>No</td><td>Holds live SSH/SFTP sockets — not serializable.</td></tr>
 *   <tr><td>{@link TerminalSessionCache}</td><td>No</td><td>Holds live WebSocket + SSH channel — not serializable.</td></tr>
 *   <tr><td>{@link TransferCache}</td><td>Yes</td><td>Plain data object; per-entry TTL maps to Redis {@code EX}. Best Redis candidate.</td></tr>
 *   <tr><td>{@link MetricsCache}</td><td>Possible</td><td>{@code SessionMetrics} must be made public + serializable first.</td></tr>
 *   <tr><td>{@link K8sClientCache}</td><td>No</td><td>Holds live KubernetesClient + SSH tunnel — not serializable.</td></tr>
 *   <tr><td>{@link DockerClientCache}</td><td>No</td><td>Holds live DockerClient + SSH tunnel — not serializable.</td></tr>
 * </table>
 */
@Configuration
...
public class RegistryCacheConfig {
     ...
     * per-entry TTL mapping to Redis EX).  Falls back to Caffeine if Redis is not configured
    ...
    public TransferCache transferCache() {
        ...
        if (redisTemplate != null && isRedisReachable(redisTemplate)) {
            ...
            return new TransferCache(
                    new RedisCache<>(redisTemplate, objectMapper, "transfer", RemoteTransfer.class));
        }
        log.info("TransferCache backed by Caffeine (max-size={})", props.getTransfer().getMaxSize());
        return new TransferCache(
