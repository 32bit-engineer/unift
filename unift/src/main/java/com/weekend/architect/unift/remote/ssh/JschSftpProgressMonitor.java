...
/**
 * Bridges JSch's {@link SftpProgressMonitor} API to our {@link TransferProgressCallback} functional
 * interface.
 *
 * <p>JSch calls {@link #count(long)} for each chunk that has been transferred. Note: JSch passes
 * the <em>delta</em> (bytes in this chunk), not the cumulative total, so we accumulate here.
 *
 * <p>When a {@link CancellationToken} is provided and cancelled, {@link #count(long)} returns
 * {@code false} — JSch's built-in signal to abort the copy loop immediately. The remote file is
 * left in a partial state; the service layer deletes it after detecting the cancellation.
 */
class JschSftpProgressMonitor implements SftpProgressMonitor {
     ...
     * @return {@code true} to continue; {@code false} cancels the transfer (JSch will stop the copy
     *     loop)
     */
    @Override
    public boolean count(long count) {
        ...
        // JSch breaks out of its write loop, closes the remote file, and put() returns
        // normally.
