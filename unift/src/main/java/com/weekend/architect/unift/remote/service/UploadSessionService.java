 ...
 *
 * <p>An upload session tracks which chunks of a large file have been acknowledged. The caller is
 * responsible for splitting the file and sending chunks through a separate channel (e.g. the SFTP
 * upload endpoints); this service tracks the metadata and completion state.
 */
public interface UploadSessionService {
     ...
     *
     * @throws com.weekend.architect.unift.remote.exception.UploadSessionNotFoundException if not
     *     found / not owned
     */
     ...
     *
     * <p>When the last outstanding chunk is acknowledged the session status automatically
     * transitions to {@code COMPLETED}.
     *
     * @throws com.weekend.architect.unift.remote.exception.UploadSessionNotFoundException if not
     *     found / not owned
