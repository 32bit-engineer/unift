...
public interface SessionAnalyticsService {
     ...
     *
     * <p>Expensive probes (SSH latency, packet loss, system metrics, CPU per node)
     * are executed in parallel on virtual threads and time-bounded so that the
     * overall call completes within a few seconds even when some probes fail.
     * The snapshot is automatically persisted to the database after assembly.
     *
     ...
     * @throws com.weekend.architect.unift.remote.exception.SessionNotFoundException   if session does not exist
     * @throws com.weekend.architect.unift.remote.exception.SessionAccessDeniedException if the caller does not own the session
     */
    SessionAnalyticsResponse getAnalytics(String sessionId, UUID requestingUserId);

    /**
     * Returns historical analytics snapshots stored for the given session,
     * newest-first.  Ownership is enforced at the DB level — a user can only
     * query their own sessions even if the session is no longer live.
     *
